...
/**
 * 사이트의 공통 설정 정보를 조회하는 로직을 담당,
 * 이건 주로 인증이 필요 없는 공개 정보를 제공
 */
@Service
@RequiredArgsConstructor
@Transactional(readOnly = true) // 이 서비스의 메서드들은 모두 조회만 하므로 readOnly = true로 설정하여 성능 최적화
public class CommonServiceImpl implements CommonService {
     ...
     * 모든 공통 설정 값을 조회하여 하나의 DTO로 만들어 반환
     */
    @Override
    ...
    /**
     * 특정 설정 키(Key)의 값(Value)을 안전하게 조회하는 헬퍼(도우미) 메서드
     * 데이터베이스에 해당 키가 존재하지 않을 경우, 미리 지정된 기본값을 대신 반환함
     *
    ...
    private String findConfigValue(String key, String defaultValue) {
        return siteConfigRepository.findById(key)      // 1. 키로 SiteConfig 엔티티를 찾음 (반환값: Optional<SiteConfig>)
                .map(SiteConfig::getConfigValue)       // 2. 찾았다면(Optional이 비어있지 않다면), 그 안의 configValue 값을 꺼냄
                .orElse(defaultValue);                 // 3. 만약 찾지 못했다면(Optional이 비어있다면), 대신 기본값을 반환함
    }
}
